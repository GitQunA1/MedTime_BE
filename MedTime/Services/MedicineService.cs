...
namespace MedTime.Services
{
    ...
    {
        private readonly MedicineRepo _repo;
        private readonly IMapper _mapper;

        public MedicineService(MedicineRepo repo, IMapper mapper)
        {
            _repo = repo;
            _mapper = mapper;
        }

        public async Task<PaginatedResult<MedicineDto>> GetAllAsync(int pageNumber, int pageSize)
        {
            var query = _repo.GetAllQuery();
            var paginatedEntities = await query.ToPaginatedListAsync(pageNumber, pageSize);
            var dtoItems = _mapper.Map<List<MedicineDto>>(paginatedEntities.Items);
        ...
        public async Task<MedicineDto?> GetByIdAsync(int id)
        {
            ...
            return _mapper.Map<MedicineDto>(entity);
        }

        public async Task<MedicineDto> CreateAsync(MedicineCreate request)
        {
        ...
        public async Task<bool> UpdateAsync(int id, MedicineUpdate request)
        {
